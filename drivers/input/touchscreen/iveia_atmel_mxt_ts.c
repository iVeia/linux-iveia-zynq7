...
static void rotate_180_deg_if_enabled(int * xpos, int xmax, int * ypos, int ymax)
{
#ifdef ROTATE_LCD_180_DEG
	*xpos = xmax - *xpos;
	*ypos = ymax - *ypos;
#endif
}
				      ...
				      u8 *message, u8 id)
{
	...
	rotate_180_deg_if_enabled(&x, data->max_x, &y, data->max_y);
...
static int mxt_read_count_messages(struct mxt_data *data, u8 count)
{
	 ...
	 * Note: reading all messages in one read may cause errors in some
	 * cases (specifically, found on v2.6.37), so we read one message at a
	 * time.
	 */
	for (i = 0;  i < count; i++) {
...
int mxt_download_config(struct mxt_data *data, const char *fn)
{
	...
	/* TODO: Not currently supported on atlas-i-lpe. */
		...
		const struct i2c_device_id *id)
{
	...
	error = mxt_interrupt(client->irq, data);
